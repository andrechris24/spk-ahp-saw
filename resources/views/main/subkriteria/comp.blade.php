...
<div class="card">
	<div class="card-header">
		<h4 class="card-title">Masukkan Perbandingan Sub Kriteria {{ $title }}</h4>
	</div>
							...
							<tr>
								...
								<th>
									<input type="radio" class="btn-check" name="subkriteria[{{ $loop->index }}]"
										...
										$value[$loop->index]['nilai'] > 0 || old('subkriteria.' . $loop->index) == 'left' ?
									'checked' : '' }}>
								...
								<td>
									<div class="input-group mb-3">
										<input type="number" name="skala[{{ $loop->index }}]" min="1" max="9" class="form-control text-center 
								...
								<th>
									<input type="radio" name="subkriteria[{{ $loop->index }}]" class="btn-check" value="right"
										id="right-{{ $loop->index }}" autocomplete="off" {{ $value[$loop->index]['nilai'] < 0 ||
