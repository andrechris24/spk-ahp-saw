...
class AdminController extends Controller
{
	...
	public function postRegister(Request $request)
	{
		...
		return back()->withInput()->with('status', 'Registrasi akun gagal');
	}
	...
	public function postLogin(Request $request)
	{
		...
		$ingat=$request->has('remember')?true:false;
		if ($cek) {
		...
		return back()->with(
			'status', 'Email ' . $requests['email'] . ' tidak ditemukan'
		);
	}
	...
	public function update(Request $request, $id)
	{
		...
		$d = User::find($id);
		if ($d == null) {
			return redirect('admin/profile/' . $id)->withInput()->with(
		...
		if ($data) {
			return redirect('admin/profile/' . $id)->with(
				'status', 'Profil sudah diupdate'
			);
		}
		return redirect('admin/profile/' . $id)->withInput()->with(
			'status', 'Profil gagal diupdate'
		);
	}
	public function postForgetPassword(Request $request)
	{
		...
		if ($result == null){
			return redirect('forget-password')->with(
				'status', 'Email ' . $currequest['email'] . ' tidak ditemukan'
			);
		}
		...
		return $status === Password::RESET_LINK_SENT
			? back()->with(
				['status' => 'Link reset password sudah dikirim. Cek folder spam jika belum masuk.']
	...
	public function postResetPassword(Request $request)
	{
		...
		$cek = User::find();
		if ($cek == null)
			return back()->with('status', 'Akun tidak ditemukan');
		...
		return $status === Password::PASSWORD_RESET
			? redirect('login')->with('status', 'Reset password berhasil. Silahkan login dengan password baru.')
