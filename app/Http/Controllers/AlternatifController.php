...
class AlternatifController extends Controller {
	...
	public function tambah(Request $altrequest) {
		...
		try {
			...
			if ($alternatif) {
				return back()->withSuccess('Alternatif sudah ditambahkan');
			}
	...
	public function update(Request $updaltrequest, $id) {
		try {
			...
			if (!$cek) {
				return back()->withError('Data Alternatif tidak ditemukan');
			}

			$req = $updaltrequest->all();
			$upd = $cek->update($req);
			if ($upd) {
				return back()->withSuccess('Data Alternatif sudah diupdate');
			}
